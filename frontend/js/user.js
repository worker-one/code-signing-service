...
const UserModule = (() => {
    ...
    const uploadAndSignFile = async () => {
        ...
        try {
            ...
            xhr.open('POST', 'http://localhost:8000/api/user/files/upload');
            ...
            xhr.onload = function() {
                if (xhr.status === 200) {
                    ...
                    downloadLink.href = `http://localhost:8000/api/user/files/download/${response.id}`;
    ...
    const loadFileHistory = async () => {
        try {
            const response = await fetch('http://localhost:8000/api/user/files/history', {
                headers: {
    ...
    const renderFileHistory = (files) => {
        ...
        files.forEach(file => {
            ...
            if (file.status === 'signed') {
                actionButton = `<a href="http://localhost:8000/api/user/files/download/${file.id}" class="btn btn-small btn-primary">Download</a>`;
